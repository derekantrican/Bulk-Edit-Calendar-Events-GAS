...
function checkUpdate() {
  var versionToCheck = SpreadsheetApp.openById("1uPzvv3ccihl_4sgJCfJES-oqRxWFAEQWE7_DJZUQpQQ").getSheetByName("Welcome!").getRange("F3").getValue();
  ...
  if (versionToCheck <= currentVersion)
    ...
    Browser.msgBox("There is a more current version available! The most recent version is v" + versionToCheck +" . Here is the link to download it: http://goo.gl/eIfKAq");
}
...
function resetFields() {
  ...
  sheet.getRange("C23").setValue("No");
